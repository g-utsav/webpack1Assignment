...
document.querySelector("#men").addEventListener("click",myFnc);
function myFnc(){
...
function cartUpdate(){
var containerCart = JSON.parse(localStorage.getItem("containerCart")) || []
document.querySelector("#check").innerHTML = ""
        var totalPrice = containerCart.reduce(function(sum , ele){
            return sum+ele.price;
        },0)
        
        var priceP = document.createElement("p");
        var len = containerCart.length
        priceP.innerText = `Items in Cart is ${len} and Total Amount is Rs ${totalPrice}`;

        var checkOut = document.createElement("div");
        checkOut.setAttribute("id","checkOut");
        checkOut.innerText = "Go To Cart!"
        localStorage.setItem("amountPayable", totalPrice);
        document.querySelector("#check").append(priceP,checkOut)
}
cartUpdate()


displayUI(mensData)
function displayUI(mensData){
...
mensData.forEach(function(ele){
    ...
    subDiv1.append(button,img)

    var h3 = document.createElement("h3")
    h3.innerText = ele.name;

    var childDiv = document.createElement("div");
    childDiv.setAttribute("id","priceDiv")
    var price = document.createElement("p")
    price.innerText = ele.price;
    var strikeOff = document.createElement("p")
    strikeOff.innerText = ele.strikedoffprice

    childDiv.append(price, strikeOff);

    subDiv2.append(h3, childDiv);

    masterDiv.append(subDiv1, subDiv2)
    document.querySelector("#container").append(masterDiv)
...
document.querySelector("#sortPrice").addEventListener("change", sortPriceCall);

function sortPriceCall(){
...
document.querySelector("#sortByName").addEventListener("change", sortNameCall);

function sortNameCall(){
